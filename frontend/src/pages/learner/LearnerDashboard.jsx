...
const globalStyles = `
  ...
  /* Tab — red underline slide-in (NavBar pattern) */
  .td-tab {
  ...
  /* Watch button — RED base, BLACK blob on hover */
  .td-watch-btn {
    position: relative; overflow: hidden;
  ...
  /* Join/link button — outline, red blob */
  .td-join-btn {
    position: relative; overflow: hidden;
  ...
  /* Recording day header — WHITE base, red left border accent, dark text */
  .rec-day-header {
  ...
  /* Settings button — white base, red blob hover */
  .ld-settings-btn {
    position: relative; overflow: hidden;
...
const getClassRecordings = (cls) => {
  if (cls.recordings?.length>0) return cls.recordings.map((r,i)=>({label:r.label||`Part ${i+1}`,url:r.url}));
...
const Badge = ({ status }) => {
  const cfg = { ongoing:{bg:'#fff0f0',color:'#E8001C',border:'#ffc0c0'}, completed:{bg:'#f3f4f6',color:'#6b7280',border:'#e5e7eb'}, scheduled:{bg:'#ffffff',color:'#374151',border:'#e5e7eb'} }[status] || {bg:'#ffffff',color:'#374151',border:'#e5e7eb'};
  return <span style={{fontFamily:'Poppins,sans-serif',fontSize:'0.7rem',fontWeight:600,padding:'0.2rem 0.65rem',borderRadius:20,background:cfg.bg,color:cfg.color,border:`1px solid ${cfg.border}`,whiteSpace:'nowrap'}}>{status}</span>;
};
...
export default function LearnerDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setStats({totalClasses:cd.length,upcomingClasses:cd.filter(c=>getAutoStatus(c)==='scheduled').length,completedClasses:cd.filter(c=>getAutoStatus(c)==='completed').length,ongoingClasses:cd.filter(c=>getAutoStatus(c)==='ongoing').length});
  ...
  const hasActiveBatch = ()=>{ const t=new Date();t.setHours(0,0,0,0); return batches.some(b=>{if(!b.endDate) return true; const e=new Date(b.endDate);e.setHours(23,59,59,999); return e>=t;}); };
  ...
  const isClassBatchActive = cls=>{
    ...
    const t=new Date();t.setHours(0,0,0,0); const e=new Date(cb.endDate);e.setHours(23,59,59,999); return e>=t;
  };

  const isBatchActive = batch=>!batch.endDate||(()=>{const t=new Date();t.setHours(0,0,0,0);const e=new Date(batch.endDate);e.setHours(23,59,59,999);return e>=t;})();
  ...
  const totalRecParts    = getClassesWithR().reduce((s,c)=>s+getClassRecordings(c).length,0);

  const getRecByDay = ()=>{
    const groups={};
    getClassesWithR().forEach(cls=>{
      const key=toDateKey(cls.date);
      if(!groups[key]) groups[key]={key,label:fmtDayLabel(cls.date),date:cls.date,items:[]};
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="block lg:hidden space-y-3">
              {classes.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(cls=>{
                const auto=getAutoStatus(cls),parts=isClassBatchActive(cls)?getClassRecordings(cls):[];
                return(
                  <div key={cls._id} className="bg-white rounded-xl p-4 border border-[#e5e7eb] hover:border-[#ffc0c0] hover:shadow-md transition-all" style={{boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
                    <div className="flex items-start justify-between mb-2">
                      <div className="min-w-0 flex-1 mr-3">
                        ...
                        <p style={{fontFamily:'Poppins,sans-serif'}} className="text-xs text-[#9ca3af] mt-1">{fmtShort(cls.date)}</p>
                    ...
                    {((cls.teamsLink&&auto!=='completed')||parts.length>0)&&(
                      <div className="flex flex-wrap gap-2 mt-2">
                        {cls.teamsLink&&auto!=='completed'&&<a href={cls.teamsLink} target="_blank" rel="noopener noreferrer" className="td-join-btn"><ExternalLink size={13}/><span>Join</span></a>}
                        {parts.map((p,i)=><a key={i} href={p.url} target="_blank" rel="noopener noreferrer" className="td-watch-btn"><PlayCircle size={13}/><span>{p.label}</span></a>)}
                        {!isClassBatchActive(cls)&&hasAnyRecording(cls)&&<span style={{fontFamily:'Poppins,sans-serif'}} className="text-xs text-[#9ca3af] italic self-center">Batch ended</span>}
                      </div>
                    )}
                  </div>
                );
                  ...
                  {classes.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(cls=>{
                    ...
                    return(
                      <tr key={cls._id} className="hover:bg-[#fff0f0] transition-colors group">
                        ...
                        <td style={{fontFamily:'Poppins,sans-serif'}} className="px-6 py-4 text-sm text-[#6b7280]">{fmtDate(cls.date)}</td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap items-center gap-2">
                            {cls.teamsLink&&auto!=='completed'&&<a href={cls.teamsLink} target="_blank" rel="noopener noreferrer" className="td-join-btn"><ExternalLink size={13}/><span>Join</span></a>}
                            {parts.map((p,i)=><a key={i} href={p.url} target="_blank" rel="noopener noreferrer" className="td-watch-btn"><PlayCircle size={13}/><span>{p.label}</span></a>)}
                            {!isClassBatchActive(cls)&&hasAnyRecording(cls)&&<span style={{fontFamily:'Poppins,sans-serif'}} className="text-xs text-[#9ca3af] italic">Batch ended</span>}
                          </div>
                        </td>
                      </tr>
                    );
            ...
            {recDayGroups.length>0?(
              <div className="space-y-3 sm:space-y-4">
                {recDayGroups.map(group=>{
                  ...
                  const partCount=group.items.reduce((s,c)=>s+getClassRecordings(c).length,0);
                  return(
                      ...
                      <button onClick={()=>toggleDay(group.key)} className="rec-day-header w-full flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="text-left min-w-0">
                            ...
                            <p style={{fontFamily:'Poppins,sans-serif',color:'#9ca3af'}} className="text-xs mt-0.5 truncate">{new Date(group.date).toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'long',day:'numeric'})}</p>
                          </div>
                        </div>
                      ...
                      {!isCollapsed&&(
                        <div className="divide-y divide-[#f5f5f5]">
                          {group.items.map((cls,idx)=>{
                            const parts=getClassRecordings(cls),auto=getAutoStatus(cls);
                            return(
                              <div key={cls._id} className="px-4 sm:px-6 py-4 sm:py-5 hover:bg-[#fff0f0] transition-colors">
